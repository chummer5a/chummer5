		...
		private void cboMetavariant_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if (cboMetavariant.SelectedValue.ToString() != "None")
			{
                ...
                if (cboHeritage.SelectedIndex < 4)
                {
                    try
                    {
                        XmlNodeList objXmlMetatypeList = objXmlDocumentPriority.SelectNodes("/chummer/priorities/priority[category = \"Heritage\" and value = \"" + ((ListItem)(cboHeritage.Items[cboHeritage.SelectedIndex + 1])).Value + "\"]/metatypes/metatype[name = \"" + lstMetatypes.SelectedValue.ToString() + "\"]");
