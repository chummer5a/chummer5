	...
	public partial class frmCreate : Form
	{
		...
		private void frmCreate_Load(object sender, EventArgs e)
		{
            ...
            if (_objCharacter.BuildMethod == CharacterBuildMethod.Priority)
            {
                ...
                lblPBuildComplexForms.Text = String.Format("{0} " + LanguageManager.Instance.GetString("String_Of") + " {1}", (_objCharacter.CFPLimit).ToString(), _objCharacter.CFPLimit.ToString());
                tabInfo.TabPages.RemoveAt(0);

                lblPBuildContacts.Text = String.Format("{0} " + LanguageManager.Instance.GetString("String_Of") + " {1}", (_objCharacter.ContactPoints).ToString(), _objCharacter.ContactPoints.ToString());
            }
            else
            {
		...
		public void UpdateCharacterInfo()
		{
			...
			if (!_blnSkipUpdate)
			{
				...
				if (_objCharacter.RESEnabled)
				{
					...
					lblLivingPersonaDeviceRating.Text = intResponse.ToString();
					if (_objCharacter.MetatypeCategory != "Technocritters")
			...
			if (treGear.SelectedNode.Level > 0)
			{
				...
				else
				{
					...
					if (objGear.IsProgram && _objOptions.CalculateCommlinkResponse)
					{
						...
						{
							if (objParent.Category == "Commlinks" || objParent.Category == "Nexus")
		...
		private bool PickGear()
		{
			...
			try
			{
				if (treGear.SelectedNode.Level > 0)
				{
					...
					if (objSelectedGear.Category == "Commlinks")
					{
						Commlink objCommlink = (Commlink)objSelectedGear;
		...
		private void MoveControls()
		{
			...
			lblLivingPersonaDeviceRating.Left = lblLivingPersonaDeviceRatingLabel.Left + lblLivingPersonaDeviceRatingLabel.Width + 6;
            lblLivingPersonaBiofeedbackFilterLabel.Left = lblLivingPersonaDeviceRating.Left + 35;
		...
		private void PopulateGearList()
		{
			...
			foreach (Cyberware objCyberware in _objCharacter.Cyberware)
			{
				if (objCyberware.AllowGear != null)
				{
					...
					{
						if (objCyberware.AllowGear["gearcategory"].InnerText == "Commlinks")
						{
							TreeNode objNode = new TreeNode();
			...
			foreach (Weapon objWeapon in _objCharacter.Weapons)
			{
				foreach (WeaponAccessory objAccessory in objWeapon.WeaponAccessories)
				{
					if (objAccessory.AllowGear != null)
					{
						...
						{
							if (objAccessory.AllowGear["gearcategory"].InnerText == "Commlinks")
							{
								TreeNode objNode = new TreeNode();
				...
				foreach (Weapon objUnderbarrel in objWeapon.UnderbarrelWeapons)
				{
					foreach (WeaponAccessory objUnderbarrelAccessory in objUnderbarrel.WeaponAccessories)
					{
						if (objUnderbarrelAccessory.AllowGear != null)
						{
							...
							{
								if (objUnderbarrelAccessory.AllowGear["gearcategory"].InnerText == "Commlinks")
								{
									TreeNode objNode = new TreeNode();
			...
			{
				bool blnAdd = true;
				if (chkCommlinks.Checked && objGear.Category != "Commlinks")
					blnAdd = false;
